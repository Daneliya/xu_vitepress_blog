...
链的输入是**含 “question” 键的字典**（`question`对应用户当前问题），输出是含 “answer”“chat_history” 等键的字典：
...
- **检索自动触发**：调用`invoke`时，链会先将`question`传入检索器，获取相关文档片段，再将 “历史对话（chat_history）+ 问题（question）+ 相关文档” 合并为提示传给 LLM；
